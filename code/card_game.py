...
def draw_players(playerName:list,playerChip:list,*args):
    # screen.blit(PLAYER_NAME_FONT.render(c,True,(0,0,0)),(640,360))

    for i in range(len(PLAYER_INFO_BAR_LIST)):
        screen.blit(PLAYER_NAME_FONT.render('hi',True,(0,0,0)),(640,360))
...
def draw_card(playerId:int,card_id:str,position:tuple,deal:bool = False,fold:bool = False):
    ...
    #player
    if playerId == 2 or playerId == -1 or True:
...
def draw_hand(handList:list[list],gameRound:int,fold = False):
    '''draw all hand card'''
    for i in range(len(PLAYER_INFO_BAR_LIST)):
        for j in range(2):
            if gameRound == 1:
                draw_card(i,handList[i][j],HAND_POSITION[i][j],True)
            draw_card(i,handList[i][j],HAND_POSITION[i][j],False,fold)
...
def check_button(playerChip:int,least_bet = 0,press = False,invalidList:list = []) -> dict[str,int]:
    '''press: True will pass,return button name\n
       invalidList: receive keyword of button'''
    choice = FOLD
...
def draw_blind(small_blind):
    last = (small_blind - 1) % 5
    pygame.draw.rect(PLAYER_INFO_BAR_LIST[last],(255, 243, 171),(0,0,30,30))
    pygame.display.flip()
